...
export class McpServer
{
	...
	*/
	...
	private async handleResourceCompletion(request: CompleteRequest, ref: ResourceReference): Promise<CompleteResult>
	{
	...
	private setResourceRequestHandlers()
	{
		...
		this.server.setRequestHandler(
			...
			async (request, extra) =>
			{
				const resources = Object.entries(this._registeredResources).filter(([_, resource]) => resource.enabled).map(
					([uri, resource]) => (
					{
						uri,
						name: resource.name,
...
export class ResourceTemplate
{
	...
	constructor(uriTemplate: string | UriTemplate, private _callbacks: 
		{
			/**
