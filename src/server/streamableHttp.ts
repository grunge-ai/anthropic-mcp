...
export interface EventStore
{
	...
	*/
	...
	replayEventsAfter(lastEventId: EventId, { send }: {send: (eventId: EventId, message: JSONRPCMessage) => Promise<void>}): Promise<StreamId>;
}
	...
	private async handlePostRequest(req: IncomingMessage, res: ServerResponse, parsedBody?: unknown): Promise<void>
	{
		try
		{
			...
			else if(hasRequests)
			{
				...
				// handle each message
				// JUST BAD CODE (CHECK IF ONMESSAGE, THEN ITERATE)
				for(const message of messages)
				{
					this.onmessage?.(message);
				}
